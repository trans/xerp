...
module Xerp::Store
  ...
  module Statements
    ...
    def self.select_blocks_by_file(db : DB::Database, file_id : Int64) : Array(BlockRow)
      results = [] of BlockRow
      db.query(<<-SQL, file_id) do |rs|
        SELECT block_id, file_id, kind, level, start_line, end_line, header_text, parent_block_id
        FROM blocks WHERE file_id = ?
      SQL
        rs.each do
          results << BlockRow.new(
            rs.read(Int64), rs.read(Int64), rs.read(String), rs.read(Int32),
            rs.read(Int32), rs.read(Int32), rs.read(String?), rs.read(Int64?)
          )
        end
      end
      results
    ...
    def self.select_block_by_id(db : DB::Database, block_id : Int64) : BlockRow?
      db.query_one?(<<-SQL, block_id, as: {Int64, Int64, String, Int32, Int32, Int32, String?, Int64?})
        SELECT block_id, file_id, kind, level, start_line, end_line, header_text, parent_block_id
